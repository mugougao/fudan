...
import { fetchBuildingPointById } from "@/api/financial/energy.ts";
import PoiLayer from "../code/PoiLayer";

class BuildingEnergyPoiLayer extends PoiLayer {
  ...
  async render(buildingId: string) {
    ...
    // this.focus(buildingId, { distanceFactor: 0.1 });
    this.flyTo(buildingId);
    return res;
  }
}

export default new BuildingEnergyPoiLayer();
