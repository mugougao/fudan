...
async function useAssetProps(campusId: CampusId) {
  ...
  // 获取校区数据，默认为邯郸校区
  const campusData = campusDataMap[campusId as unknown as keyof typeof campusDataMap] || campusDataMap["3"];
...
async function useElectProps(campusId: CampusId) {
  ...
  // 获取校区数据，默认为邯郸校区
  const overviewData = campusEnergyMap[campusId as unknown as keyof typeof campusEnergyMap] || campusEnergyMap["3"];
...
async function useWaterProps(campusId: CampusId) {
  ...
  // 获取校区数据，默认为邯郸校区
  const overviewData = campusWaterMap[campusId as unknown as keyof typeof campusWaterMap] || campusWaterMap["3"];
...
export function useOverviewCampus(categories: MaybeRefOrGetter<string | undefined>) {
  ...
  const {
    ...
    async () => {
      ...
      // 获取校区数据，默认为邯郸校区
      return campusOverviewMap[campusId.value as unknown as keyof typeof campusOverviewMap] || campusOverviewMap["3"];
    },
    {},
    {
