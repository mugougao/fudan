...
<template>
  <div
    class="view-panel pointer-events-none wh-full flex flex-col pb-65px">
    <header class="relative mt-50px h-80px flex-shrink-0 children:pointer-events-auto">
      <template v-if="showReturn">
        <slot name="return" :return-handler="returnHandler">
          <button
            type="button"
            class="absolute bottom-3 right-5 flex-center rounded-xl bg-transparent px-2 py-1 text-14px text-gray-500 text-white hover:bg-gray-300/20"
            @click="returnHandler">
    ...
    <div class="flex flex-auto">
      <TransitionFadeInLeft>
        <div
          ...
          class="view-panel-left h-full overflow-hidden transition-all children:pointer-events-auto"
          :class="showDataPanel ? 'w-450px pl-5' : 'w-0'">
          <div class="h-full w-430px" :class="leftGrid && 'grid grid-rows-24 grid-cols-1 gap-15px'">
      ...
      <TransitionFadeInRight>
        <div
          ...
          :class="showDataPanel ? 'w-450px pr-5' : 'w-0'">
          <div class="h-full w-430px" :class="rightGrid && 'grid grid-rows-24 grid-cols-1 gap-15px'">
