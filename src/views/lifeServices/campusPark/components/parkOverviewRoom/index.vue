          ...
          itemStyle: {
            normal: {
              color: {
                ...
                y2: 0,
                colorStops: [
                  { offset: 0, color: "#03F4FF" },
                  { offset: 0.3, color: "#07BCFF" },
                  { offset: 0.7, color: "#08A0FF" },
                  { offset: 1, color: "#0979FF" },
                ],
                global: false,
              },
            },
  ...
  <div class="relative h-180px w-full flex items-center justify-center">
    <div class="relative h-160px w-160px">
      ...
      <div class="swivel rotate absolute left-80px top-1/2 h-0 w-73px flex origin-left-center justify-end" :style="style">
        <div class="outRound flex-center translate-x-12px border -translate-y-12px">
          <div class="inrRound" />
        </div>
      </div>

      <div class="absolute left-1/2 top-40px flex items-baseline text-16px text-white -translate-x-1/2">
        <GradientText class="text-30px font-bold font-number" :deg="0" :colors="{ 0: '#06C0FF', 100: '#FFFFFF' }">
          {{ props.value }}
        </GradientText>
        <span>%</span>
      </div>
      <div class="absolute left-1/2 top-85px whitespace-nowrap text-18px text-white font-text-bold -translate-x-1/2">
        {{ $t('dormitory.area.roomOccupancy') }}
      </div>
    </div>
  </div>
</template>
