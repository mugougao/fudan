...
const option = computed(() => {
  ...
  const highlightPercent = total ? round((highlightValue / total) * 100, 1) : 0;
  return {
    color,
    title: [
      {
        ...
        textStyle: {
          rich: {
            value: { align: "center", fontSize: 22, color: "#fff", fontFamily: "D-DIN" },
            unit: { align: "center", fontSize: 14, color: "#fff", fontFamily: "D-DIN" },
          },
        },
      },
      {
        ...
        top: "52%",
        left: 60,
        textStyle: { align: "center", fontSize: 12, color: "#fff", fontFamily: "AlibabaPuHuiTi-3-bold" },
      },
    ],
    tooltip: {
      trigger: "item",
    ...
    legend: {
      ...
      itemWidth: 6,
      itemHeight: 6,
      textStyle: { fontSize: 15, padding: [0, 0, 0, 5], fontFamily: "AlibabaPuHuiTi-3-bold" },
      data: data.map((item, index) => ({ name: item.name, color: color[index] })),
    },
    series: [
      {
        name: "main-out",
