...
const { chartData = [], total = [] } = defineProps<{
  chartData?: { name: string; value: number }[];
  total?: { yesterday: number; month: number; year: number }[];
}>();

const headCells = ["", "", "用电量（万Kwh）", "金额（万元）"];
...
<template>
  <div class="electric-info pt-3">
    <UiThirdTitle title="用电信息" />
    <div class="chart-box mt-2 flex">
      ...
      <div class="h-[170px] w-1/2">
        <UiChartLine :data="chartData" unit="万Kwh" color="yellow" />
      </div>
    </div>
  </div>
</template>
