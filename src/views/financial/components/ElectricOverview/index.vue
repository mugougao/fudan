...
defineOptions({ name: "ElectricOverview", inheritAttrs: false });
...
const columns = [
  { title: "financial.index.yesterday", kwfField: "nowelect", moneyField: "nowelectamount", rateField: "nowelectmom", tooltip: "日环比增长率" },
  { title: "financial.index.month", kwfField: "monthelect", moneyField: "monthelectamount", rateField: "monthelectmom", tooltip: "月环比增长率" },
  { title: "financial.index.year", kwfField: "yearelect", moneyField: "yearelectamount", rateField: "yearelectmom", tooltip: "年环比增长率" },
];
    ...
    <div class="electric-overview-item relative h-[100px] flex items-center whitespace-nowrap">
      ...
      <div class="content-box ml-1 h-[100px] flex flex-col flex-auto justify-between px-2.5">
        <span class="mt-2 text-[14px] text-white/60">
          {name}
          {t("financial.index.electricityUse")}
        </span>

        <span class="flex items-center justify-between">
          <span>
            <span class="text-[20px] text-white/80 font-number">{numberToThousands(kwh)}</span>
            <span class="ml-0.5 text-[12px] text-[#9E9E9E]">万Kwh</span>
          </span>
...
<template>
  <div :class="cn('electric-overview', $attrs.class ?? '')">
